...
@interface MXMail_MJRefreshFooter()
/** 显示状态文字的标签 */
...
@implementation MXMail_MJRefreshFooter
#pragma mark - 懒加载
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 初始化文字
        [self setTitle:MXMail_MJRefreshFooterStateIdleText forState:MJRefreshFooterStateIdle];
        [self setTitle:MXMail_MJRefreshFooterStateRefreshingText forState:MJRefreshFooterStateRefreshing];
        [self setTitle:MXMail_MJRefreshFooterStateNoMoreDataText forState:MJRefreshFooterStateNoMoreData];
    }
    return self;
}
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    // 旧的父控件
    [self.superview removeObserver:self forKeyPath:MXMail_MJRefreshContentSize context:nil];
    [self.superview removeObserver:self forKeyPath:MXMail_MJRefreshPanState context:nil];
    
    if (newSuperview) { // 新的父控件
        // 监听
        [newSuperview addObserver:self forKeyPath:MXMail_MJRefreshContentSize options:NSKeyValueObservingOptionNew context:nil];
        [newSuperview addObserver:self forKeyPath:MXMail_MJRefreshPanState options:NSKeyValueObservingOptionNew context:nil];
        
        self.mj_h = MXMail_MJRefreshFooterHeight;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if (self.state == MJRefreshFooterStateIdle) {
        ...
        if ([keyPath isEqualToString:MXMail_MJRefreshPanState]) {
        ...
        } else if ([keyPath isEqualToString:MXMail_MJRefreshContentOffset]) {
    ...
    // 不管是什么状态，都要调整位置
    if ([keyPath isEqualToString:MXMail_MJRefreshContentSize]) {
        [self adjustFrameWithContentSize];
    }
}
