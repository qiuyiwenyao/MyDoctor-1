...
@interface MXMail_MJRefreshHeader()
/** 显示上次刷新时间的标签 */
...
@implementation MXMail_MJRefreshHeader
#pragma mark - 懒加载
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        // 设置默认的dateKey
        self.dateKey = MXMail_MJRefreshHeaderUpdatedTimeKey;
        
        // 设置为默认状态
        self.state = MJRefreshHeaderStateIdle;
        
        // 初始化文字
        [self setTitle:MXMail_MJRefreshHeaderStateIdleText forState:MJRefreshHeaderStateIdle];
        [self setTitle:MXMail_MJRefreshHeaderStatePullingText forState:MJRefreshHeaderStatePulling];
        [self setTitle:MXMail_MJRefreshHeaderStateRefreshingText forState:MJRefreshHeaderStateRefreshing];
    }
    return self;
}
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    if (newSuperview) {
        self.mj_h = MXMail_MJRefreshHeaderHeight;
    }
}
...
{
    _dateKey = dateKey ? dateKey : MXMail_MJRefreshHeaderUpdatedTimeKey;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:MXMail_MJRefreshContentOffset]) {
    ...
    switch (state) {
        case MJRefreshHeaderStateIdle: {
            if (oldState == MJRefreshHeaderStateRefreshing) {
                ...
                // 恢复inset和offset
                [UIView animateWithDuration:MXMail_MJRefreshSlowAnimationDuration delay:0.0 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
//                    _scrollView.mj_insetT -= self.mj_h;
        ...
        case MJRefreshHeaderStateRefreshing: {
            [UIView animateWithDuration:MXMail_MJRefreshFastAnimationDuration delay:0.0 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                // 增加滚动区域
