...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        */
        ...
        CGRect closeFrame = self.closeButton.frame;
    closeFrame.size = CGSizeMake(27, 27);
    closeFrame.origin.x = self.containerView.frame.size.width - 42;
...
//    containerViewRect.origin.y = round(CGRectGetMidY(self.window.bounds) - CGRectGetMidY(containerViewRect));
    containerViewRect.origin.y = appHeight * 0.60;
...
//            self.containerView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 1, 1);
            self.containerView.frame  =CGRectMake(0, appHeight, appWidth, 0);
            
        } completion:^(BOOL finished){
            [UIView animateWithDuration:kTransformPart1AnimationDuration delay:0 options:UIViewAnimationOptionTransitionCurlUp animations:^{
